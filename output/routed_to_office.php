	...
	<div class="container-report-delin">
		...
		<table class="table-report" border="1">
			<tr align="center" style="font-weight: bold;">
				...
				<td>Doc. Type</td>
				<td>Source</td>
				<td width="400px;">Communication Summary</td>
			...
			<?php
				...
				//old query
				//$qry_to_office = mysqli_query($connection,"select * from tbl_document_transaction a, tbl_document b where a.route_office_code='$_REQUEST[office_code]' and a.current_action='REL' and a.barcode=b.barcode");

				//yants
				//modification
				//May 7, 2019
				//To check query for document has been done and need to do

				if($_REQUEST['action']=='todo'){
					$qry_to_office = mysqli_query($connection,"select * from tbl_document_transaction a, tbl_document b where a.route_office_code='$_REQUEST[office_code]' and a.current_action='REL' and a.barcode=b.barcode");
				} else if($_REQUEST['action']=='done'){
					$qry_to_office = mysqli_query($connection,"select * from tbl_document_transaction a, tbl_document b where a.office_code='$_REQUEST[office_code]' and (a.current_action='REC') and a.barcode=b.barcode");
				} else {
					$qry_to_office = mysqli_query($connection,"select * from tbl_document_transaction a, tbl_document b where a.office_code='$_REQUEST[office_code]' and (a.current_action='END') and a.barcode=b.barcode");
				}

				for($a=1;$a<=mysqli_num_rows($qry_to_office);$a++){
					$rows = mysqli_fetch_assoc($qry_to_office);
					echo "<tr>";
